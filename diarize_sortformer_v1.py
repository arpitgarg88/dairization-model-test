...
MODEL_SLUG = "diar-sortformer-4spk-v1"
BACKEND = "nemo-cpu"
BENCHMARK_RESULT = "benchmark_diar_sortformer_4spk_v1.json"
...
def run_smoke_test(audio_path: Path, device: str | None = None) -> None:
    ...
    model = load_sortformer_model(MODEL_ID, device=device or "cpu")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="NVIDIA Sortformer 4spk v1 (offline)")
    ...
    if args.benchmark:
        run_benchmark(
            MODEL_ID,
            BENCHMARK_RESULT,
            tqdm_desc="diar-sortformer-4spk-v1",
            streaming_offline=False,
